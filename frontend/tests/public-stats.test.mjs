...
test('PublicStatsStrip component exists with degrade contract', () => {
  ...
  assert.ok(
    strip.includes('public.stats') || strip.includes('getTranslations'),
    'PublicStatsStrip must use public.stats i18n',
  );
  assert.ok(
    strip.includes('return null') || strip.includes('unavailable'),
    'PublicStatsStrip must hide or show unavailable on failure (D-12)',
  );
});
...
test('BFF GET /api/public/stats proxies with X-Forwarded-For', () => {
  ...
  assert.ok(
    route.includes('X-Forwarded-For') || route.includes('x-forwarded-for'),
    'BFF must forward client IP for rate limiting',
  );
  assert.ok(
    route.includes('/api/v1/public/stats'),
    'BFF must proxy to FastAPI public stats endpoint',
  );
  assert.ok(route.includes('Retry-After'), 'BFF must propagate Retry-After on 429');
});
