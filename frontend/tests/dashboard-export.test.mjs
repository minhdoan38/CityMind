...
test('export BFF route exists and streams with officer auth', () => {
  ...
  assert.ok(src.includes('getClaims'), 'export route must gate with getClaims');
  assert.ok(src.includes('officerFetch'), 'export route must use officerFetch');
  assert.ok(
    src.includes('/api/v1/reports/export'),
    'export route must proxy FastAPI /api/v1/reports/export',
  );
  assert.ok(
    /Content-Disposition/i.test(src),
    'export route must pass through Content-Disposition',
  );
  assert.ok(
    !src.includes('.json()'),
