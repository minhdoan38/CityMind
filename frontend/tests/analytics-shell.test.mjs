...
test('analytics page uses officerFetch and URL date keys', () => {
  const page = fs.readFileSync(
    path.resolve(root, 'src/app/dashboard/analytics/page.tsx'),
    'utf8',
  );
  assert.ok(page.includes('officerFetch'), 'analytics page must use officerFetch');
  assert.ok(page.includes('/api/v1/analytics'), 'analytics page must call analytics API');
